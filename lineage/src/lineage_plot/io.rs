//! Parquet reads for `senna lineage-plot` — every file this figure depends on is loaded
...
/// `[x, y, dx, dy]`, written by `senna lineage --layout umap`) and project each into
...
/// One MST edge, with the velocity call `senna lineage` made for it.
