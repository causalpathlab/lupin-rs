//! `senna lineage-plot` — render the outputs of `senna lineage` into a
...
//! `senna lineage --markers` (with the default `--layout phate`) plus the
...
///
/// There are as many MST nodes as `senna lineage --n-centroids` asked for (200 by
...
/// *clusters*; `senna lineage` defaults to `K = min(cells/10, 200)`, a fine mesh.
...
pub struct LineagePlotArgs {
    ...
    #[arg(
        ...
        long_help = "Which trajectory nodes carry a cell_type label.\n\
                     `senna lineage` emits one MST node per --n-centroids (200 by default),\n\
