//! Loading for `senna dyn-assoc`: the lineage's per-cell pseudotime + branch, and the
...
///
/// **Cells the lineage could not place on any tree are dropped here.** `senna lineage`
...
/// aggregation in `senna dyn-assoc`. `ids[c]` is the contiguous cell-type id of lineage cell
...
/// written by `senna lineage --markers`, or a user-supplied override — and align it to
