//! Entry point for `senna dyn-assoc` — modality dynamics along the lineage. Loads the
...
//! [`crate::assoc`].
//!
//! When the lineage was annotated (`senna lineage --markers`, which writes
...
pub struct AssocArgs {
    ...
    #[arg(
        ...
        long_help = "cell<TAB>cell_type TSV for the cell-type-level report.\n\
                     ...
                     written by `senna lineage --markers`.\n\
                     \n\
...
pub fn run_assoc(args: &AssocArgs) -> Result<()> {
    ...
    // Cell-type-level report (optional; requires `senna lineage --markers`).
    if !args.no_celltype {
    ...
    if !std::path::Path::new(&path).exists() {
        ...
        info!(
            "cell-type report skipped: no annotation at {path} \
             (run `senna lineage --markers`, or pass --celltype-annot)"
        );
        return Ok(());
    }
