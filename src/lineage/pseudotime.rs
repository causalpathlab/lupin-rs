...
use legume_numeric::matrix::dense_mat_io::{axis_id_names, read_mat, Mat, MatWithNames};
...
/// the root, and compute per-cell pseudotime. No I/O.
...
pub struct PseudotimeArgs {
    ...
    #[arg(
        long = "from",
        help = "Run manifest (or its prefix) to read the cell table from",
        long_help = "When given, the cell table is the manifest's outputs.cell_embedding,\n\
                     else outputs.latent.\n\
...
fn project_centroids_to_2d(
    ...
    // The nearest-node search is O(N·K·D) and independent per cell; the
    // accumulation stays serial so the sums keep their order.
    let nearest: Vec<usize> = (0..latent.nrows())
        .into_par_iter()
        .map(|i| closest_node_to_row(latent, i, graph))
        .collect();
    for (i, &node) in nearest.iter().enumerate() {
    ...
    let row_names = axis_id_names("node_", k);
...
/// Pseudotime for each of `cell_names`, matched by row name, from a table
/// [`write_pseudotime`] wrote (the `pseudotime` column, else the first).
/// Cells absent from the table get NaN.
/// `(min, span)` over the finite values, erroring when there is no range to
/// colour by (all NaN, or constant).
pub fn finite_range(vals: &[f32]) -> anyhow::Result<(f32, f32)> {
    let (lo, hi) = vals
        .iter()
...
pub fn read_pseudotime_for(path: &str, cell_names: &[Box<str>]) -> anyhow::Result<Vec<f32>> {
    let MatWithNames { rows, cols, mat } =
        Mat::from_parquet(path).map_err(|e| anyhow::anyhow!("reading pseudotime {path}: {e}"))?;
    let j = cols
        .iter()
    ...
    anyhow::ensure!(
        mat.ncols() > j,
        "pseudotime parquet {path} has no data column"
    );
    let by_cell: rustc_hash::FxHashMap<&str, f32> = rows
        .iter()
        .enumerate()
        .map(|(i, c)| (c.as_ref(), mat[(i, j)]))
        .collect();
    let mut n_missing = 0usize;
    let out: Vec<f32> = cell_names
        .iter()
        .map(|c| {
    ...
    if n_missing > 0 {
        log::warn!(
            "{path}: {n_missing}/{} cells have no pseudotime row",
            cell_names.len()
        );
    }
    Ok(out)
...
fn write_graph_nodes(nodes: &Mat, feat_names: &[Box<str>], path: &str) -> anyhow::Result<()> {
    let row_names = axis_id_names("node_", nodes.nrows());
...
fn write_graph_edges(edges: &[(usize, usize)], weights: &[f32], path: &str) -> anyhow::Result<()> {
    ...
    let row_names = axis_id_names("edge_", edges.len());
