...
pub enum ColorBy {
    ...
    Annotation,
    /// Continuous scalar from `lupin pseudotime`'s `.pseudotime.parquet`
...
pub struct PlotArgs {
    ...
    #[arg(
        long,
        help = "Pseudotime parquet from `lupin pseudotime` (cells × 1)",
        long_help = "Pseudotime parquet from `lupin pseudotime` (cells × 1).\n\
            ...
            let path = resolved.pseudotime.as_ref().ok_or_else(|| {
                anyhow::anyhow!(
                    "--colour-by pseudotime requires --pseudotime PATH \
                     (or `lupin pseudotime --from manifest` to populate \
                     manifest.pseudotime.pseudotime)"
                )
            })?;
            let norm = read_pseudotime_normalized(path, &cell_names)?;
            let g: Vec<i64> = norm
                .iter()
...
fn resolve_inputs(args: &PlotArgs) -> anyhow::Result<ResolvedInputs> {
    ...
    // has the Reingold-Tilford tree layout populated by `lupin pseudotime`,
...
fn read_pseudotime_normalized(path: &str, cell_names: &[Box<str>]) -> anyhow::Result<Vec<f32>> {
    let vals = crate::lineage::pseudotime::read_pseudotime_for(path, cell_names)?;
    let (lo, span) = crate::lineage::pseudotime::finite_range(&vals)?;
    Ok(vals
        .into_iter()
        .map(|v| {
            if v.is_finite() {
